...
const TRANSCRIPT_DIR = process.env.AGENT_HUB_HARNESS_DIR
  ? path.join(process.env.AGENT_HUB_HARNESS_DIR, 'transcripts')
  : null;
let currentRef = null;
...
const ADDITIONAL_DIRS = JSON.parse(process.env.AGENT_HUB_ADDITIONAL_DIRS || '[]');   // extra roots the core passed through
...
// A provider the core injects is named the same in every harness: `hub-<id>`.
...
function startPi(resumeRef) {
  ...
  // The core hands the user project dir via AGENT_HUB_CWD; run pi there so its
